...
# 1. Pipeline de Dados (Foco em ConvNeXt)
transform = {
    'train': transforms.Compose([
        transforms.Resize((232, 232)), # ConvNeXt costuma usar crop de 224 a partir de 232
...
def main():
    print(f"\n[ESTÁTICO AVANÇADO] Treinando ConvNeXt Tiny no RAF-DB")
    model = get_convnext()
    ...
    for param in model.head.parameters(): # ConvNeXt usa 'head' em vez de 'classifier' no timm
    ...
    # ConvNeXt prefere AdamW e LRs menores
    ...
    for epoch in range(30):
        ...
        if v_acc > best_acc:
            best_acc = v_acc
            torch.save(model.state_dict(), os.path.join(MODELS_DIR, "convnext_rafdb_best.pth"))
            
    print(f"\n[FINALIZADO] Melhor Acurácia ConvNeXt: {best_acc:.4f}")
    
    # Métricas Finais
    model.load_state_dict(torch.load(os.path.join(MODELS_DIR, "convnext_rafdb_best.pth")))
    model.eval()
    y_true, y_pred = [], []
    with torch.no_grad():
    ...
    pd.DataFrame(report).transpose().to_csv(os.path.join(REPORTS_DIR, "metrics_convnext_rafdb.csv"))
    
    # Matriz de Confusão
    cm = confusion_matrix(y_true, y_pred, normalize='true')
    plt.figure(figsize=(10, 8))
    sns.heatmap(cm, annot=True, fmt='.2f', cmap='Blues', xticklabels=EMOTION_CLASSES, yticklabels=EMOTION_CLASSES)
    plt.title('Matriz de Confusão ConvNeXt Tiny - RAF-DB')
    plt.savefig(os.path.join(FIGURES_DIR, "cm_convnext_rafdb.png"))
    plt.close()
